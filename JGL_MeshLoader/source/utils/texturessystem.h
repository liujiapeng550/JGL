...
public:
	static unsigned int getTextureId(char const* path, bool isflip=true,GLint tex_wrapping= GL_REPEAT)
	{
...
// -Z (back)
// -------------------------------------------------------
	static unsigned int loadCubemap(std::vector<std::string> faces,bool isflip)
	{
		unsigned int textureID;
		glGenTextures(1, &textureID);
